...
def parse_chords_to_midi(chords: List[str]) -> str:
    ...
    return (
        "Convert the following chord symbols to its composite Midi Note representation. "
        "Only answer with the Midi Note representation per chord in the format 'Note,Note,Note' "
        "separating chords by newlines.\n"
        f"{'chr(10)'.join(chord.strip() for chord in chords)}"
    )
...
def create_midi_file(parsed_chords, output_file="chord_progression.mid"):
    ...
    track = 0
    _time = 0
    midi.addTrackName(track, _time, "Chord Progression")
    midi.addTempo(track, _time, 60)  # Slower tempo (60 BPM)

    # Set the instrument to Rhodes Piano (MIDI program 4)
    midi.addProgramChange(track, 0, _time, 4)

    for chord in parsed_chords:
        notes = [int(note) for note in chord.split(",")]
        for note in notes:
            midi.addNote(
                track, 0, note, _time, 2, 80
            )  # Longer duration (2 beats) and slightly lower velocity
        _time += 2  # Move to the next chord after 2 beats
...
if __name__ == "__main__":
    ...
    genre = (
        input("Enter the genre of the song (or press Enter to skip): ").strip() or None
    )
    ...
    parsed_chords = parse_chords_to_midi(
        [chord for chord in progression.split("|") if chord.strip()]
    ).split("\n")
